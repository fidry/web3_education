...
from libs.eth_async.data.models import TxArgs, Networks, TokenAmount
...
class OfficialBridge(Base):
    async def deposit(self, amount: TokenAmount | None = None) -> str:
        if not amount:
            amount = Base.get_eth_amount_for_bridge()

        failed_text = 'Failed to bridge ETH to zkSync Era via the official bridge'
        logger.info(f'Start to bridge {amount.Ether} ETH from Ethereum to ZkSync via official bridge')
