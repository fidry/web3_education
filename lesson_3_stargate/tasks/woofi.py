...
class WooFi(Base):
    ...
    async def swap_usdc_to_arb(self, amount: Optional[TokenAmount] = None, slippage: float = 1) -> str:
        ...
        arb_price = await self.get_token_price(token_symbol='ARB')
        min_to_amount = TokenAmount(
        ...
        receipt = await tx.wait_for_receipt(client=self.client, timeout=200)
        if receipt:
            return f'{amount.Ether} USDC was swaped to {min_to_amount.Ether} ARB via WooFi: {tx.hash.hex()}'

        return f'{failed_text}!'

    async def swap_arb_to_wbtc(self, amount: Optional[TokenAmount] = None, slippage: float = 1) -> str:
        failed_text = 'Failed swap ARB to WBTC via WooFi'
        contract = await self.client.contracts.get(contract_address=Contracts.ARBITRUM_WOOFI)
        from_token = Contracts.ARBITRUM_ARB
        ...
        arb_price = await self.get_token_price(token_symbol='ARB')
        wbtc_price = await self.get_token_price(token_symbol='WBTC')

        min_to_amount = TokenAmount(
            amount=float(amount.Ether) * (arb_price / wbtc_price) * (1 - slippage / 100),
        ...
        receipt = await tx.wait_for_receipt(client=self.client, timeout=200)
        if receipt:
            return f'{amount.Ether} ARB was swaped to {min_to_amount.Ether} WBTC via WooFi: {tx.hash.hex()}'
