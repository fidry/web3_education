...
class Stargate(Base):
    ...
    async def send_usdc_from_avalanche_to_usdt_bsc(
        ...
        try:
            ...
            native_balance = await self.client.wallet.balance()
            if native_balance.Wei < value.Wei:
                return (f'{failed_text}: To low native balance: balance: {native_balance.Ether}; '
                        f'value: {value.Ether}')
            ...
            receipt = await tx.wait_for_receipt(client=self.client, timeout=300)
            if receipt:
                return (f'{amount.Ether} USDC was send from {self.client.network.name} to '
                        f'{to_network.name} via Stargate: {tx.hash.hex()}')
            return f'{failed_text}!'
