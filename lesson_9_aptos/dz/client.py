...
from data.models import TokenAmount, ResourceType, Tokens, Token, SwapRouters, SwapRouterInfo, DomainNamesInfo
from data.config import logger
from utils.utils import get_explorer_hash_link, prepare_address_for_aptoscan_api
from functions.functions_utils import my_cach
...
class AptosClient(RestClient):
    ...
    def get_token_price(self, amount_in: TokenAmount, from_token: Token, to_token: Token = Tokens.APT) -> TokenAmount:
        try:
    ...
    def get_account_domain_names(self, account_address: Optional[AccountAddress] = None) -> int:
        ...
        for resource in response.json():
            ...
            if ResourceType.token_store in resource_type or DomainNamesInfo.router in resource_type:
                domain_names += 1
        return domain_names
    ...
    def account_resource(
        ...
        if self.proxy is None:
            ...
            response = requests.get(request, proxies={'https': f'http://{self.proxy}'})
